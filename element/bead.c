...
//這裡是把滑鼠的 pixel 位置 ➜ 轉成盤面格子座標：
    ...
    mouse_was_down = mouse_down;//記憶滑鼠狀態（防多次觸發）
    if(mouse_down && 240 < mstate.x && mstate.x < 290 && 225 < mstate.y && mstate.y < 290)//拖曳時間延長（按到特定區）
    {
...
void Bead_draw(Elements *self) {
    ...
    al_draw_scaled_bitmap(
        ...
        0
    );//這是用 Allegro 畫圖的標準做法。你是從 obj->img 中整張貼圖，依照珠子自己的位置 (x, y) 和尺寸 (w, h) 畫上去。
    al_draw_filled_rectangle(240, 225, 290, 275, al_map_rgb(100, 100, 100));/*這段畫出一個灰色矩形，應該是某個 UI 按鈕背景或「拖曳延長區」。*/
